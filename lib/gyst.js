...
export function mint(typeCode, namespace = IX_NS, entropy) {
  ...
  let rest;
  if (entropy && String(entropy).length >= 4) {
    const raw = strip(String(entropy));
    rest = (/^[0-9a-f]+$/.test(raw) ? raw : toHexBytes(String(entropy))).slice(0, 19);
...
export function route(parsed) {
  ...
  if (ns >= 0x1c1 && ns <= 0x1cf) return { handler: "Partner", block: "inbound" };
  if (ns >= 0x1b1 && ns <= 0x1bf) return { handler: "Catalog", block: "vendor" };
  if (t === 0x219) return { handler: "Rip", block: "recycle" };
...
export function seedMint(typeCode, namespace, entropyHex) {
  ...
  const raw = strip(String(entropyHex || ""));
  const rest = (/^[0-9a-f]+$/.test(raw) ? raw : toHexBytes(String(entropyHex || "seed"))).padEnd(19, "0").slice(0, 19);
  const h = (toHex(type, 3) + toHex(ns, 3) + toHex(ts, 6) + "8" + rest).slice(0, 32).padEnd(32, "0");
  return parse(h);
}
