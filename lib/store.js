...
export function StoreProvider({ children }) {
  ...
  useEffect(() => {
    if (!hydrated) return;
    localStorage.setItem(KEY, JSON.stringify({ parts, builds, shipments, events, activeGroup }));
  }, [parts, builds, shipments, events, activeGroup, hydrated]);
  ...
  const api = useMemo(() => {
    ...
    const planShip = (build) => {
      ...
      const ship = {
        uuid: mint(TYPE.shipment, IX_NS).formatted,
    ...
    return { addParts, openBuild, planShip, reset, log };
  }, [parts, activeGroup]);
